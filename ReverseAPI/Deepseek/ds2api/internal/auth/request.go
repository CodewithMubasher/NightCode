...
type BrowserTokenFunc func(ctx context.Context) (string, bool)

type Resolver struct {
	Store            *config.Store
	Pool             *account.Pool
	Login            LoginFunc
	BrowserToken     BrowserTokenFunc
...
func (r *Resolver) SetBrowserTokenFunc(fn BrowserTokenFunc) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.BrowserToken = fn
}

func (r *Resolver) getBrowserToken(ctx context.Context) (string, bool) {
	r.mu.Lock()
	fn := r.BrowserToken
	r.mu.Unlock()
	if fn == nil {
...
func (r *Resolver) ensureManagedToken(ctx context.Context, a *RequestAuth) error {
	if r.Store.AuthMode() == "browser" {
		if token, ok := r.getBrowserToken(ctx); ok && strings.TrimSpace(token) != "" {
			a.Account.Token = token
			a.DeepSeekToken = token
			return nil
		}
		return fmt.Errorf("browser token not yet available — wait for login or restart")
	}
