...
func (c Config) MarshalJSON() ([]byte, error) {
	...
	if strings.TrimSpace(c.BrowserAuth.Mode) != "" || strings.TrimSpace(c.BrowserAuth.BrowserPath) != "" || c.BrowserAuth.RefreshIntervalSec > 0 || c.BrowserAuth.Headless {
		m["browser_auth"] = c.BrowserAuth
	}
...
func (c *Config) UnmarshalJSON(b []byte) error {
	...
	for k, v := range raw {
		...
		case "browser_auth":
			if err := json.Unmarshal(v, &c.BrowserAuth); err != nil {
				return fmt.Errorf("invalid field %q: %w", k, err)
			}
		case "compat":
