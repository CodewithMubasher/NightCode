...
func (s *Store) AuthMode() string {
	s.mu.RLock()
	defer s.mu.RUnlock()
	mode := strings.ToLower(strings.TrimSpace(s.cfg.BrowserAuth.Mode))
	if mode == "" {
		return "api"
	}
	return mode
...
func (s *Store) BrowserAuthConfig() BrowserAuthConfig {
	s.mu.RLock()
	defer s.mu.RUnlock()
	cfg := s.cfg.BrowserAuth
