...
func (p *CloudflareProvider) streamOnce(ctx context.Context, req ChatRequest, events chan<- ChatEvent) *ProviderError {
	messages := convertCloudflareMessages(req)
	toolInfos := convertCloudflareToolSpecs(req.Tools)
...
func convertCloudflareMessages(req ChatRequest) []*schema.Message {
...
func classifyCloudflareError(err error) *ProviderError {
	msg := err.Error()
...
func convertCloudflareToolSpecs(specs []ToolSpec) []*schema.ToolInfo {
	if len(specs) == 0 {
		return nil
	}
