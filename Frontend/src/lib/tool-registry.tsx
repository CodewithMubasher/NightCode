...
const toolRenderers: Record<string, ToolRenderer> = {
    ...
    renderDetail: (entry) => {
      ...
      return (
        <div className="mt-2 text-xs">
          {filePath && (
            <div className="text-white/40 mb-1 font-mono">{filePath}</div>
          )}
          {typeof output === "string" && output && (
            <pre className="rounded-lg bg-white/5 border border-white/10 p-3 overflow-x-auto text-white/70 font-mono text-[11px] leading-4 max-h-48 overflow-y-auto">
              {output}
            </pre>
          )}
        </div>
  ...
  shell: {
    ...
    getFileName: (entry) => {
      ...
      if (entry.input && typeof entry.input === "object" && "command" in entry.input) {
        return String((entry.input as { command: string }).command)
      }
      return undefined
    },
    renderDetail: (entry) => {
      ...
      return (
        <div className="mt-2 text-xs">
          <pre className="rounded-lg bg-white/5 border border-white/10 p-3 overflow-x-auto text-white/70 font-mono text-[11px] leading-4">
            <span className="text-white/40">$ </span>{cmd}
          </pre>
          {typeof output === "string" && output && (
            <pre className="mt-1 rounded-lg bg-white/5 border border-white/10 p-3 overflow-x-auto text-white/60 font-mono text-[11px] leading-4 max-h-48 overflow-y-auto">
              {output}
            </pre>
          )}
        </div>
